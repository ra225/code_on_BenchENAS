...
def parse_nvidia_info(gpu_enabled_list, nvidia_info):
    ...
    for line in list1:
        ...
        if len(line_split) <= 1:
            ...
            num_gpu = num_gpu + 1
        elif line_split[1][-1] != "%" and line_split[0][-1] != "%":
            #GPU Fan 100%
            remove_list.append(line)
    ...
    for i in range(num_gpu):
        ...
        # print("list1=",list1)
        # print("list2=", list2)
        # print("list1[2 * i]=",list1[2 * i])
        # print("list1[2 * i + 1]=",list1[2 * i + 1])
...
def detect_gpu():
    ...
    for each_work in gpu_info.keys():
        ...
        print(type(gpus_info))
        print("gpus_info:",gpus_info)
        for each_info in gpus_info:
            _item = {
                'worker_ip': ip,
                'gpu_id': each_info['gpu_slot'],
                'status': 0 if each_info['left_mem'] > each_info['total_mem']*0.4 else 1,
            ...
            if each_info["job_list"] != '[]':
                job_list = each_info["job_list"]
                job_list = job_list.replace('"', "'")
                job_list = eval(job_list)
                print(type(job_list))
                ...
                if _item['status'] == 0:
                    for each_job in job_list:
                        import re
                        print(each_job)
                        if re.search('py36',each_job["process_name"]):
                            available_num -= 1
                            _item['status'] = 1
                            break
            #if _item['status'] == 0:
