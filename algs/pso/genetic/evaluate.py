    ...
    def evaluate(self):
        ...
        for indi in self.individuals:
            ...
            if _key in _map:
                _count += 1
                _error = _map[_key]
                self.log.info('Hit the cache for %s, key:%s, error:%.5f, assigned_error:%.5f'%(indi.id, _key, _error, indi.error_mean))
                CacheToResultFile.do(indi.id, _error)
                indi.acc_mean = _error
