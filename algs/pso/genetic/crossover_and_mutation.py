...
import numpy as np
import copy
from algs.pso.genetic.population import Individual
...
class CrossoverAndMutation(object):
    def __init__(self, prob_crossover, prob_mutation, _log, pops, gen_no, _params):
    ...
    def process(self, gbest):
        crossover = Crossover(self.pops, self.prob_crossover, self.crossover_eta, self.acc_mean_threshold,
                              self.complexity_threshold, self.log)
        offspring = crossover.do_crossover(gbest)
...
class Crossover(object):
    def __init__(self, pops, prob_, eta, acc_mean_threshold, complexity_threshold, _log):
        self.pops = pops
        self.prob = prob_
        self.eta = eta
    ...
    def do_crossover(self, gbest):
        new_offspring_list = []
        for now_id in range(len(self.pops.individuals)):
            ind1 = self.pops.individuals[now_id]
            if gbest == False:
                ind2 = self.pops.best_indi[now_id]
            else:
                ind2 = self.pops.gbest_indi
                if ind2 == None:
                    ind2 = self.pops.best_indi[now_id]
