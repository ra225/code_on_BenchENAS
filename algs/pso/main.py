...
from algs.pso.utils import Utils
from algs.pso.genetic.statusupdatetool import StatusUpdateTool
from algs.pso.genetic.population import Population
from algs.pso.genetic.evaluate import FitnessEvaluate
from algs.pso.genetic.crossover_and_mutation import CrossoverAndMutation
from algs.pso.genetic.selection_operator import Selection
...
class EvolveCNN(object):
    ...
    def fitness_evaluate(self):
        ...
        for indi in self.pops.individuals:
            ...
            if indi.error_mean == -1 and indi.id in fitness_map.keys():
                indi.error_mean = fitness_map[indi.id]
            if indi.error_mean != -1:
                if self.pops.gbest_fitness > indi.error_mean:
                    self.pops.gbest_fitness = indi.error_mean
                    self.pops.gbest_indi = indi
                if self.pops.best_indi[now].error_mean > indi.error_mean:
                    self.pops.best_indi[now] = indi

    def crossover_and_mutation(self, gbest):
        params = {}
        ...
        cm = CrossoverAndMutation(self.params['genetic_prob'][0], self.params['genetic_prob'][1], Log,
                                  self.pops, self.pops.gen_no, params)
        offspring = cm.process(gbest)
    ...
    def do_work(self, max_gen):
       ...
       # Log.info('EVOLVE[%d-gen]-Finish the evaluation' % (gen_no))
        ...
        #    self.crossover_and_mutation(curr_gen%2==1)
            self.crossover_and_mutation(False)
            self.crossover_and_mutation(True)
