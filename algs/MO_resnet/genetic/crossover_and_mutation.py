...
import numpy as np
import copy
from algs.MO_resnet.genetic.population import Individual
from algs.MO_resnet.utils import Utils
from algs.MO_resnet.genetic.statusupdatetool import StatusUpdateTool
from algs.MO_resnet.genetic.population import ConvUnit, PoolUnit, FcUnit
...
class Crossover(object):
    ...
    def _choose_two_parents(self, wh):
        ...
        idx1 = random.randint(0, max(self.T-1,1))
        idx2 = random.randint(0, max(self.T-1,1))
        while idx2 == idx1:
            idx2 = random.randint(0, max(self.T-1,1))
    ...
    def do_crossover(self):
        ...
        for i in range(len(self.individuals)):
            ...
            ind1_list = ind1.__list__()
            ind2_list = ind2.__list__()
            self.log.info('Do crossover on indi:%s and indi:%s' % (ind1.id, ind2.id))
            p1, p2 = copy.deepcopy(ind1), copy.deepcopy(ind2)
            # l1, l2 = copy.deepcopy(ind1_list), copy.deepcopy(ind2_list)
            # for different unit, we define two list, one to save their index and the other one save unit
            p1_conv_index_list_x = []
            p1_conv_index_list_y = []
            p1_conv_layer_list = []
            p1_pool_index_list_x = []
            p1_pool_index_list_y = []
            p1_pool_layer_list = []
            p1_full_index_list = []
            p1_full_layer_list = []

            p2_conv_index_list_x = []
            p2_conv_index_list_y = []
            p2_conv_layer_list = []
            p2_pool_index_list_x = []
            p2_pool_index_list_y = []
            p2_pool_layer_list = []
            p2_full_index_list = []
            p2_full_layer_list = []

            i = -1
            for one_block in p1.units:
                i += 1
                if isinstance(one_block, list):
                    j = -1
                    for unit in one_block:
                        j += 1
                        if unit.type == 1:
                            p1_conv_index_list_x.append(i)
                            p1_conv_index_list_y.append(j)
                            p1_conv_layer_list.append(unit)
                        elif unit.type == 2:
                            p1_pool_index_list_x.append(i)
                            p1_pool_index_list_y.append(j)
                            p1_pool_layer_list.append(unit)
                else:
                    p1_full_index_list.append(i)
                    p1_full_layer_list.append(one_block)

            i = -1
            for one_block in p2.units:
                i += 1
                if isinstance(one_block, list):
                    j = -1
                    for unit in one_block:
                        j += 1
                        if unit.type == 1:
                            p2_conv_index_list_x.append(i)
                            p2_conv_index_list_y.append(j)
                            p2_conv_layer_list.append(unit)
                        elif unit.type == 2:
                            p2_pool_index_list_x.append(i)
                            p2_pool_index_list_y.append(j)
                            p2_pool_layer_list.append(unit)
                else:
                    p2_full_index_list.append(i)
                    p2_full_layer_list.append(one_block)
            '''
            i = -1
            for one_block in ind1_list:
                for one_layer in one_block:
                    i += 1
                    if isinstance(one_layer, list):
                        if one_layer[0] == 1:
                            a = ConvUnit()
                            a.__parse__(one_layer)
                            p1_conv_index_list.append(i)
                            p1_conv_layer_list.append(a)
                        elif one_layer[0] == 2:
                            a = PoolUnit()
                            a.__parse__(one_layer)
                            p1_pool_index_list.append(i)
                            p1_pool_layer_list.append(a)
                    else:
                        a = FcUnit()
                        a.__parse__(one_block)
                        p1_full_index_list.append(i)
                        p1_full_layer_list.append(a)
                        break

            i = -1
            for one_block in ind2_list:
                for one_layer in one_block:
                    i += 1
                    if isinstance(one_layer, list):
                        if one_layer[0] == 1:
                            ...
                            p2_conv_layer_list.append(a)
                        elif one_layer[0] == 2:
                            a = PoolUnit()
                            a.__parse__(one_layer)
                            p2_pool_index_list.append(i)
                            p2_pool_layer_list.append(a)
                    else:
                        a = FcUnit()
                        a.__parse__(one_block)
                        p2_full_index_list.append(i)
                        p2_full_layer_list.append(a)
                        break
            '''
            ...
            for i in range(l):
                ...
                if _p < self.prob:
                    ...
                    n_k1 = int(np.power(2, int(n_k1)))
                    n_k2 = int(np.power(2, int(n_k2)))
            ...
            for i in range(len(p1_conv_layer_list)):
                unit_list1[p1_conv_index_list_x[i]][p1_conv_index_list_y[i]] = p1_conv_layer_list[i]
            for i in range(len(p1_pool_layer_list)):
                unit_list1[p1_pool_index_list_x[i]][p1_pool_index_list_y[i]] = p1_pool_layer_list[i]
            for i in range(len(p1_full_layer_list)):
                unit_list1[p1_full_index_list[i]] = p1_full_layer_list[i]

            unit_list2 = p2.units
            for i in range(len(p2_conv_layer_list)):
                unit_list2[p2_conv_index_list_x[i]][p2_conv_index_list_y[i]] = p2_conv_layer_list[i]
            for i in range(len(p2_pool_layer_list)):
                unit_list2[p2_pool_index_list_x[i]][p2_pool_index_list_y[i]] = p2_pool_layer_list[i]
            for i in range(len(p2_full_layer_list)):
...
class Mutation(object):
    ...
    def do_some(self, indi, mutation_list, _stat_param, units_list, cur_unit):
        is_new = False
        p_ = np.random.random()
        if p_ < 0.5:
            is_new = True
            ...
            if mutation_type == 0:
                ...
                if current_conv_and_pool_length < max_length:
                    _stat_param['ADD'] += 1
                    units_list.append(
                        self.generate_a_new_layer(indi, cur_unit.type, len(indi.units)))
        ...
        for indi in self.individuals:
            ...
            if p_ < self.prob:
                ...
                for i in range(len(indi.units) - 1):
                    sum_unit.append(0)
                    if isinstance(indi.units[i], list):
                        for j in range(len(indi.units[i])):
                            cur_unit = indi.units[i][j]
                            sum_unit[i] += 1
                            self.do_some(indi, mutation_list, _stat_param, units_list, cur_unit)
                    else:
                ...
                if is_new:
                    ...
                    units_list = Individual.update_all_channels_in_mutation(units_list, 1, self.log)
                    common_layers = 0
                    while (units_list[common_layers].type != 3):
                        common_layers += 1
                    now = 0
                    i = 0
                    indi.units = []
                    while now != common_layers and i < len(sum_unit):
                        next_now = min(common_layers, now+sum_unit[i])
                        indi.units.append(units_list[now:next_now])
                        now = next_now
                        i += 1
                    if now != common_layers:
                        indi.units.append(units_list[now:common_layers])
                    last = common_layers
                    while last < len(units_list):
                        indi.units.append(units_list[last])
                        last += 1
    ...
    def alter_pool_unit(self, indi, unit, eta):
        ...
        new_ksize = int(np.power(2, int(new_ksize)))
